...
pub fn verify_update_header<V, +Ed25519<V>>(
    verifier: @V,
    ...
    verify_commit_against_trusted(verifier, untrusted, trusted, options);
}
...
pub fn verify_misbehaviour_header<V, +Ed25519<V>>(
    verifier: @V,
    untrusted: UntrustedBlockState,
    trusted: TrustedBlockState,
    options: Options,
...
pub fn verify_commit_against_trusted<V, +Ed25519<V>>(
    verifier: @V, untrusted: UntrustedBlockState, trusted: TrustedBlockState, options: Options,
) {
    let trusted_next_height = trusted.height.checked_add(1);
    assert(trusted_next_height.is_some(), CometErrors::OVERFLOWED_BLOCK_HEIGHT);
    if untrusted.height() == @trusted_next_height.unwrap() {
        check_signers_overlap(
            verifier, untrusted.signed_header.clone(), untrusted.validators.clone(),
        );
    } else {
...
pub fn check_enough_trust_and_signers<V, +Ed25519<V>>(
    verifier: @V,
...
pub fn check_signers_overlap<V, +Ed25519<V>>(
    verifier: @V, untrusted_header: SignedHeader, untrusted_validators: ValidatorSet,
) {
    let power = voting_power_in(verifier, untrusted_header, untrusted_validators, TWO_THIRDS);
    assert(power.has_enough_power(), CometErrors::INSUFFICIENT_VOTING_POWER);
}

fn voting_power_in<V, +Ed25519<V>>(
    verifier: @V,
    ...
    voting_power_in_impl(
        verifier, ref votes, validator_set.clone(), trust_threshold, validator_set.total_power(),
    )
}

fn voting_power_in_sets<V, +Ed25519<V>>(
    verifier: @V,
    signed_header: SignedHeader,
...
fn voting_power_in_impl<V, +Ed25519<V>>(
    verifier: @V,
    ...
    for validator in validator_set.validators {
        if votes.has_voted(verifier, @validator) {
