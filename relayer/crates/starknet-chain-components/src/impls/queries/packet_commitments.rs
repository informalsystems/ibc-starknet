...
pub struct QueryStarknetPacketCommitments;

impl<Chain, Counterparty, Encoding> PacketCommitmentsQuerier<Chain, Counterparty>
    for QueryStarknetPacketCommitments
where
        ...
        + HasEncodedType<Encoded = Vec<Felt>>,
{
    async fn query_packet_commitments(
        ...
        port_id: &IbcPortId,
    ) -> Result<(Vec<IbcSequence>, u64), Chain::Error> {
        let context = RpcContext::new();
        let event_filter = GetEventsInput {
            filter: EventFilter {
                chunk_size: test_utils::NUM_EVENTS,
                ..Default::default()
            },
        };
        let get_events_result = get_events(context, event_filter)
            .await
            .map_errr(Chain::raise_error)?;
