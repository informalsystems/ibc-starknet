...
pub async fn fork_starknet_chain_with_sequencer_key(
    starknet_chain_driver: &StarknetChainDriver,
    sequencer_private_key: &Felt,
) -> Result<StarknetChainDriver, HermesError> {
    let runtime = starknet_chain_driver.chain.runtime.clone();
    let chain_home_dir = starknet_chain_driver.chain_store_dir.clone();
    let chain_node_config = starknet_chain_driver.node_config.clone();

    let starknet_pid = starknet_chain_driver
        .chain_processes
        .first()
        .expect("Failed to retrieve Starknet chain process")
        .id()
    ...
    let fork_chain_home_dir = chain_home_dir.as_path().join("fork-madara");
    let mut fork_chain_node_config = chain_node_config.clone();
    fork_chain_node_config.sequencer_private_key = *sequencer_private_key;
    fork_chain_node_config.rpc_port += 20;
    let fork_rpc_port = fork_chain_node_config.rpc_port;
    ...
    let mut forked_chain_processes = fork_bootstrap
        .start_chain_forked_full_nodes(
            &fork_chain_home_dir,
            &fork_chain_node_config,
            &starknet_chain_driver.genesis_config,
            &starknet_chain_home_dir,
            "10", // FIXME: Retrieve the correct block number for `--backup-every-n-blocks`
        )
        .await?;
    ...
    let mut forked_json_rpc_url = starknet_chain_driver.chain.json_rpc_url.clone();
    let current_port = forked_json_rpc_url
        .port()
        .expect("Failed to extract port from JSON url");
    forked_json_rpc_url
    ...
    let mut forked_feeder_gateway_url = starknet_chain_driver.chain.feeder_gateway_url.clone();
    let current_port = forked_feeder_gateway_url
        .port()
        .expect("Failed to extract port from Feeder gateway url");
    forked_feeder_gateway_url
    ...
    let forked_starknet_chain = StarknetChain {
        fields: Arc::new(StarknetChainFields {
            runtime: runtime.clone(),
            chain_id: starknet_chain_driver.chain.chain_id.clone(),
            starknet_client: forked_starknet_rpc_client,
            rpc_client,
            json_rpc_url: forked_json_rpc_url,
            feeder_gateway_url: forked_feeder_gateway_url,
            ibc_client_contract_address: starknet_chain_driver
                .chain
                .ibc_client_contract_address
                .clone(),
            ibc_core_contract_address: starknet_chain_driver
                .chain
                .ibc_core_contract_address
                .clone(),
            ibc_ics20_contract_address: starknet_chain_driver
                .chain
                .ibc_ics20_contract_address
                .clone(),
            event_encoding: starknet_chain_driver.chain.event_encoding.clone(),
            poll_interval: starknet_chain_driver.chain.poll_interval,
            block_time: starknet_chain_driver.chain.block_time,
            nonce_mutex: Arc::new(Mutex::new(())),
            signers: starknet_chain_driver.chain.signers.clone(),
            client_refresh_rate: starknet_chain_driver.chain.client_refresh_rate,
            signer_mutex: Arc::new(Mutex::new(0)),
        }),
    };

    Ok(StarknetChainDriver {
        runtime: starknet_chain_driver.runtime.clone(),
        chain: forked_starknet_chain,
        chain_store_dir: fork_chain_home_dir,
        genesis_config: starknet_chain_driver.genesis_config.clone(),
        node_config: fork_chain_node_config,
        wallets: starknet_chain_driver.wallets.clone(),
        chain_processes: forked_chain_processes,
        relayer_wallet_1: starknet_chain_driver.relayer_wallet_1.clone(),
        relayer_wallet_2: starknet_chain_driver.relayer_wallet_2.clone(),
        user_wallet_a: starknet_chain_driver.user_wallet_a.clone(),
        user_wallet_b: starknet_chain_driver.user_wallet_b.clone(),
    })
}
